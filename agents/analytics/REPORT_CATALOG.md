...
| Prompt CTR | Growth | Are prompts driving clicks? | daily | category, surface | count(`prompt_click`) / count(`prompt_impression`) | `prompt_click`, `prompt_impression` | <=15 min |
| Prompt Pass-through Rate | Growth | Are users skipping prompt slides? | daily | category, surface | count(`prompt_pass_through`) / count(`prompt_impression`) | `prompt_pass_through`, `prompt_impression` | <=15 min |
| Prompt Auth Start Rate | Growth | Do prompt clicks start auth? | daily | category, surface | count(`prompt_auth_start`) / count(`prompt_impression`) | `prompt_auth_start`, `prompt_impression` | <=15 min |
