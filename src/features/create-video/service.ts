...
function mapRow(row: CreateVideoProjectRow): CreateVideoProjectDto {
  ...
  if (timeline.version !== 'create_video_v1') {
    timeline = { version: 'create_video_v1', playheadSeconds: 0, clips: [], stills: [], graphics: [], logos: [], lowerThirds: [], audioTrack: null }
  }
  return {
    id: Number(row.id),
    status: row.status,
...
export async function createOrGetActiveProjectForUser(userId: number): Promise<{ created: boolean; project: CreateVideoProjectDto }> {
  ...
  try {
    const created = await repo.create({
      ...
      timelineJson: JSON.stringify({ version: 'create_video_v1', playheadSeconds: 0, clips: [], stills: [], graphics: [], logos: [], lowerThirds: [], audioTrack: null }),
    })
