...
export default function ReviewMenu(props: { onNavigate?: () => void }) {
    ...
    ;(async () => {
      try {
        const [gRes, cRes] = await Promise.all([
          fetch('/api/space/review/groups', { credentials: 'same-origin' }),
          fetch('/api/space/review/channels', { credentials: 'same-origin' }),
        ])
  ...
  const items = [
    { label: 'Group Review', href: '/space/review/groups', count: groupCount },
    { label: 'Channel Review', href: '/space/review/channels', count: channelCount },
  ]
