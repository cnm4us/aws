...
productionsRouter.post('/api/productions', requireAuth, async (req, res, next) => {
  try {
    ...
    const { uploadId, name, config, musicUploadId, logoUploadId, logoConfigId, audioConfigId, lowerThirdConfigId, profile, quality, sound } = parsed.data
    const currentUserId = req.user!.id
    const result = await prodSvc.create(
      { uploadId, name, config, musicUploadId, logoUploadId, logoConfigId, audioConfigId, lowerThirdConfigId, profile, quality, sound },
      currentUserId
    )
