...
Locate Latest
1) List files matching agents/handoff/Handoff_*.md.
...
Create Next
1) Set M = N + 1.
2) Create agents/handoff/Handoff_{M}.md.
...
Template — agents/handoff/Handoff_{N}.md
```
Handoff {N}
...
- git add <paths>
- git commit -m "<subject>" [-m "short description"]
...
- Only create commits when the user explicitly requests a commit or Git history change, and leave `git push` to the developer.
