import { listGlobalFeedRows, getGlobalFeedPinnedRowByProductionUlid, getSpaceFeedPinnedRowByProductionUlid } from './repo'
...
export async function getGlobalFeed(opts: { userId?: number | null; limit?: number; cursor?: string | null }): Promise<FeedResponse> {
  ...
  const items = await Promise.all(rows.map(async (row) => {
    ...
    const publication = {
      ...
      has_story: Boolean(row.has_story),
      story_preview: row.story_preview == null ? null : String(row.story_preview),
...
export async function getPinnedGlobalFeedItem(opts: { userId?: number | null; productionUlid: string }): Promise<FeedResponse['items'][number] | null> {
  const productionUlid = String(opts.productionUlid || '').trim()
  if (!productionUlid) return null
  const row = await getGlobalFeedPinnedRowByProductionUlid({ productionUlid, userId: opts.userId })
  if (!row) return null
