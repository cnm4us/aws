...
export async function can(userId: number, permission: Permission, options: CanOptions = {}): Promise<boolean> {
  ...
  const spaceScoped = new Set<string>([
    PERM.SPACE_MANAGE, PERM.SPACE_SETTINGS_UPDATE, PERM.SPACE_ASSIGN_ROLES, PERM.SPACE_INVITE, PERM.SPACE_KICK, PERM.SPACE_VIEW_PRIVATE, PERM.SPACE_VIEW_HIDDEN, PERM.SPACE_POST,
    PERM.VIDEO_REVIEW_SPACE, PERM.VIDEO_APPROVE_SPACE, PERM.VIDEO_PUBLISH_SPACE, PERM.VIDEO_UNPUBLISH_SPACE,
    PERM.COMMENT_DELETE_ANY, PERM.COMMENT_MODERATE,
    PERM.MOD_COMMENT_CREATOR, PERM.MOD_SUSPEND_POSTING, PERM.MOD_BAN,
