...
| Message CTR | Growth | Are in-feed messages driving clicks? | daily | category, surface | count(`prompt_click`) / count(`prompt_impression`) | legacy canonical events: `prompt_click`, `prompt_impression` | <=15 min |
| Message Pass-through Rate | Growth | Are users skipping in-feed messages? | daily | category, surface | count(`prompt_pass_through`) / count(`prompt_impression`) | legacy canonical events: `prompt_pass_through`, `prompt_impression` | <=15 min |
| Message Auth Start Rate | Growth | Do message clicks start auth? | daily | category, surface | count(`prompt_auth_start`) / count(`prompt_impression`) | legacy canonical events: `prompt_auth_start`, `prompt_impression` | <=15 min |
