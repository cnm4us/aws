import fs from 'fs'
import path from 'path'
import { getPool } from '../../db'
import type { AssemblyAiTranscriptV1Input, S3Pointer } from '../../features/media-jobs/types'
import { ASSEMBLYAI_POLL_INTERVAL_MS, ASSEMBLYAI_POLL_TIMEOUT_SECONDS, ASSEMBLYAI_PRESIGN_TTL_SECONDS, UPLOAD_BUCKET } from '../../config'
import { presignGetObjectUrl } from '../../services/s3Presign'
import { createTranscript, fetchVtt, waitForTranscript, type AssemblyAiTelemetryContext } from '../../services/assemblyai'
import { uploadTextToS3 } from '../../services/mediaJobs/s3Logs'
import { MEDIA_JOBS_LOGS_BUCKET } from '../../config'
import * as captionsRepo from '../../features/captions/repo'
import { withExternalSpan } from '../../lib/externalObservability'

function appendLog(filePath: string | undefined, line: string) {
  if (!filePath) return
  try {
    fs.appendFileSync(filePath, `${line}\n`, 'utf8')
  } catch {}
}

async function resolveTranscriptionSource(productionId: number): Promise<{ source: S3Pointer; sourceKind: 'mastered_mp4' | 'upload_video' }> {
  const db = getPool()

  // Prefer the mastered MP4 (ffmpeg stage) when present so transcription matches the audio that MC packages.
  try {
    const [rows] = await db.query(
      `SELECT result_json
         FROM media_jobs
        WHERE status = 'completed'
          AND type IN ('audio_master_v1','video_master_v1')
          AND CAST(JSON_UNQUOTE(JSON_EXTRACT(input_json, '$.productionId')) AS UNSIGNED) = ?
        ORDER BY id DESC
        LIMIT 1`,
      [productionId]
    )
    const row = (rows as any[])[0]
    if (row?.result_json) {
      let result: any = row.result_json
      if (typeof result === 'string') {
        try { result = JSON.parse(result) } catch { result = null }
      }
      const bucket = String(result?.output?.bucket || '').trim()
      const key = String(result?.output?.key || '').trim()
      if (bucket && key) {
        return { source: { bucket, key }, sourceKind: 'mastered_mp4' }
      }
    }
  } catch {
    // ignore and fall back to upload pointer
  }

  const [rows] = await db.query(
    `SELECT u.s3_bucket, u.s3_key
       FROM productions p
       JOIN uploads u ON u.id = p.upload_id
      WHERE p.id = ?
      LIMIT 1`,
    [productionId]
  )
  const row = (rows as any[])[0]
  if (!row) throw new Error('production_not_found')
  const bucket = String(row.s3_bucket || '').trim()
  const key = String(row.s3_key || '').trim()
  if (!bucket || !key) throw new Error('missing_upload_pointer')
  return { source: { bucket, key }, sourceKind: 'upload_video' }
}

export async function runAssemblyAiTranscriptV1Job(
  input: AssemblyAiTranscriptV1Input,
  logPaths?: { stdoutPath?: string; stderrPath?: string },
  telemetry?: AssemblyAiTelemetryContext
): Promise<{
  productionId: number
  transcriptId: string
  source: S3Pointer
  sourceKind: 'mastered_mp4' | 'upload_video'
  vtt: { localPath: string; s3?: { bucket: string; key: string } | null }
}> {
  const productionId = Number(input.productionId)
  if (!Number.isFinite(productionId) || productionId <= 0) throw new Error('bad_production_id')

  appendLog(logPaths?.stdoutPath, `assemblyai_transcript_v1 start productionId=${productionId}`)

  const resolved = await resolveTranscriptionSource(productionId)
  appendLog(logPaths?.stdoutPath, `source ${resolved.sourceKind} s3://${resolved.source.bucket}/${resolved.source.key}`)

  const audioUrl = await presignGetObjectUrl({
    bucket: resolved.source.bucket,
    key: resolved.source.key,
    expiresInSeconds: ASSEMBLYAI_PRESIGN_TTL_SECONDS,
  })

  const callTelemetry: AssemblyAiTelemetryContext = {
    attrs: {
      'app.operation': 'mediajobs.attempt.process',
      mediajob_type: 'assemblyai_transcript_v1',
      production_id: productionId,
      ...(telemetry?.attrs || {}),
    },
  }

  const { transcriptId } = await withExternalSpan(
    {
      spanName: 'external.assemblyai.transcript.turnaround',
      provider: 'assemblyai',
      operation: 'transcript.turnaround',
      attrs: {
        'app.operation': 'external.assemblyai.transcript.turnaround',
        mediajob_type: 'assemblyai_transcript_v1',
        production_id: productionId,
        ...(callTelemetry.attrs || {}),
      },
    },
    async (span) => {
      const { id: transcriptId } = await createTranscript(audioUrl, callTelemetry)
      appendLog(logPaths?.stdoutPath, `transcriptId=${transcriptId}`)
      const done = await waitForTranscript(transcriptId, {
        pollIntervalMs: ASSEMBLYAI_POLL_INTERVAL_MS,
        timeoutSeconds: ASSEMBLYAI_POLL_TIMEOUT_SECONDS,
      }, callTelemetry)
      if (done.status !== 'completed') {
        appendLog(logPaths?.stderrPath, `assemblyai_failed status=${done.status} error=${done.error || ''}`)
        throw new Error(`assemblyai_failed:${done.error || 'error'}`)
      }
      span.setAttribute('app.outcome', 'success')
      return { transcriptId }
    }
  )

  const vtt = await fetchVtt(transcriptId, callTelemetry)

  const outDir = path.join(process.cwd(), 'logs', 'assemblyai')
  fs.mkdirSync(outDir, { recursive: true })
  const localPath = path.join(outDir, `production_${productionId}.vtt`)
  fs.writeFileSync(localPath, vtt, 'utf8')
  appendLog(logPaths?.stdoutPath, `wrote_vtt ${localPath}`)

  // Persist to a stable S3 location for use by the app (Plan 45).
  const stableBucket = String(UPLOAD_BUCKET || resolved.source.bucket || '').trim()
  const stableKey = `captions/vtt/production_${productionId}.vtt`
  let stableS3: { bucket: string; key: string } | null = null
  try {
    stableS3 = await uploadTextToS3(stableBucket, stableKey, vtt, 'text/vtt; charset=utf-8')
    await captionsRepo.upsertProductionCaptions({
      productionId,
      provider: 'assemblyai',
      transcriptId,
      format: 'vtt',
      language: 'en',
      bucket: stableS3.bucket,
      key: stableS3.key,
      status: 'ready',
    })
  } catch (e: any) {
    appendLog(logPaths?.stderrPath, `captions_persist_failed ${(e?.message || e)}`)
    try {
      await captionsRepo.upsertProductionCaptions({
        productionId,
        provider: 'assemblyai',
        transcriptId,
        format: 'vtt',
        language: 'en',
        bucket: stableBucket || resolved.source.bucket,
        key: stableKey,
        status: 'failed',
      })
    } catch {}
  }

  let s3Ptr: { bucket: string; key: string } | null = null
  try {
    s3Ptr = await uploadTextToS3(MEDIA_JOBS_LOGS_BUCKET, `assemblyai/production_${productionId}.vtt`, vtt, 'text/vtt; charset=utf-8')
  } catch {}

  return {
    productionId,
    transcriptId,
    source: resolved.source,
    sourceKind: resolved.sourceKind,
    vtt: { localPath, s3: stableS3 || s3Ptr },
  }
}
