...
| Message CTR | Growth | Are in-feed messages driving clicks? | daily | campaign key, surface | (count(`message_click_primary`) + count(`message_click_secondary`)) / count(`message_impression`) | `message_click_primary`, `message_click_secondary`, `message_impression` | <=15 min |
