...
export async function list(params: { status?: string | string[]; kind?: string; imageRole?: string; videoRole?: 'source' | 'export'; isSystem?: boolean | 0 | 1; userId?: number; spaceId?: number; cursorId?: number; limit: number }): Promise<UploadRow[]> {
  const db = getPool()
  const where: string[] = []
  const args: any[] = []
  if (params.status) {
  ...
  if (params.videoRole) { where.push('video_role = ?'); args.push(String(params.videoRole)) }
